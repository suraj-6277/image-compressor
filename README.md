...
2. Vercel will detect the project and use `vercel.json` for the build.
3. Add the environment variable `VITE_API_URL` with value `/api`.
4. Optionally add `MONGO_URI` to enable MongoDB metadata persistence.

> The frontend is built from `frontend/package.json`, and the backend runs as a serverless API from `backend/api.js`.
